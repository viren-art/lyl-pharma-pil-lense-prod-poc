...
export default function ResultsPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8">
      ...
      {/* Download button for Assess Variation tracked changes */}
      {workflowType === 'assess_variation' && rawResult.trackedChangesDocxBase64 && (
        <div className="mb-6 flex flex-wrap gap-3">
          <button
            onClick={() => {
              const link = document.createElement('a');
              link.href = `data:application/vnd.openxmlformats-officedocument.wordprocessingml.document;base64,${rawResult.trackedChangesDocxBase64}`;
              link.download = `PIL_Variation_Report_${new Date().toISOString().split('T')[0]}.docx`;
              link.click();
            }}
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-navy-600 hover:bg-navy-700 text-white rounded-lg font-medium text-sm transition-colors shadow-sm"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
            Download Tracked Changes Report
          </button>
        </div>
      )}
